        ...
        <footer className='sticky bottom-0'>
            <Container>
                <div className='hidden max-md:block '>
                    <ul className='flex items-center justify-between px-4 py-3 border-y-2 border-zinc-700 gap-6 font-semibold bg-zinc-950'>
                        {navItems.map((item) =>
                            item.active ? (
                                <li onClick={() => navigate(item.slug)} className='text-white cursor-pointer border-zinc-200 hover:bg-zinc-200 hover:text-black border-2 rounded-2xl px-2 py-1' key={item.name}>
                                    {item.name}
                                </li>
                            ) : null
                        )}
